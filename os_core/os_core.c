...
internal B32 os_append_data_to_file_path(String8 path, String8 data) {
  ...
  if (data.size != 0) {
    OS_Handle file =
        os_file_open(OS_AccessFlag_Write | OS_AccessFlag_Append, path);
    if (!os_handle_match(file, os_handle_zero())) {
      good = 1;
...
internal OS_Handle os_cmd_line_launch(String8 string) {
  ...
  if (parts.node_count != 0) {
    ...
    if (stdout_path.size != 0) {
      OS_Handle file =
          os_file_open(OS_AccessFlag_Write | OS_AccessFlag_Read, stdout_path);
      os_file_close(file);
      stdout_handle = os_file_open(
          OS_AccessFlag_Write | OS_AccessFlag_Append | OS_AccessFlag_ShareRead |
