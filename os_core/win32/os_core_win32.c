...
internal void os_w32_file_properties_from_attribute_data(
  ...
  os_w32_dense_time_from_file_time(&properties->created,
                                   &attributes->ftCreationTime);
  os_w32_dense_time_from_file_time(&properties->modified,
                                   &attributes->ftLastWriteTime);
  properties->flags = os_w32_file_property_flags_from_dwFileAttributes(
...
internal U32 os_get_process_start_time_unix(void) {
  ...
  FILETIME user_time;
  if (GetProcessTimes(handle, &start_time, &exit_time, &kernel_time,
                      &user_time)) {
...
#pragma warning(disable : 6320 6322)
    __try {
      RaiseException(0x406D1388, 0, sizeof(info) / sizeof(void *),
...
internal U64 os_file_read(OS_Handle file, Rng1U64 rng, void *out_data) {
  ...
  // rjf: read loop
  {
    ...
    for (U64 off = rng.min; total_read_size < to_read;) {
      ...
      ReadFile(handle, (U8 *)out_data + total_read_size, amt32, &read_size,
               &overlapped);
      off += read_size;
...
internal U64 os_file_write(OS_Handle file, Rng1U64 rng, void *data) {
  ...
  for (;;) {
    ...
    BOOL success = WriteFile(win_handle, bytes_src, write_size, &bytes_written,
                             &overlapped);
    if (success == 0) {
      break;
    }
...
internal FileProperties os_properties_from_file(OS_Handle file) {
  ...
  if (info_good) {
    ...
    props.flags =
        os_w32_file_property_flags_from_dwFileAttributes(info.dwFileAttributes);
  }
  return props;
}
...
internal B32 os_file_reserve_size(OS_Handle file, U64 size) {
  ...
  BOOL is_reserved = SetFileInformationByHandle(
      handle, FileAllocationInfo, &alloc_info, sizeof(alloc_info));
  return is_reserved;
}
...
internal String8 os_full_path_from_path(Arena *arena, String8 path) {
  ...
  DWORD path16_size =
      GetFullPathNameW((WCHAR *)path16.str, buffer_size, buffer, NULL);
...
internal B32 os_file_path_exists(String8 path) {
  ...
  B32 exists = (attributes != INVALID_FILE_ATTRIBUTES) &&
...
internal FileProperties os_properties_from_file_path(String8 path) {
  ...
  if (handle != INVALID_HANDLE_VALUE) {
    ...
    os_w32_dense_time_from_file_time(&props.modified,
    ...
    for (; last_slash_pos < path.size;
         last_slash_pos = FindSubstr8(path, Str8Lit("/"), last_slash_pos + 1,
    ...
    for (U64 off = 0; off < (U64)length;) {
      ...
      String8 next_drive_string =
          str8_from_16(scratch.arena, next_drive_string_16);
      next_drive_string = str8_chop_last_slash(next_drive_string);
      if (Str8Match(path_trimmed, next_drive_string,
...
internal OS_Handle os_file_map_open(OS_AccessFlags flags, OS_Handle file) {
  OS_Handle map = {0};
  {
    ...
    HANDLE map_handle =
        CreateFileMappingA(file_handle, 0, protect_flags, 0, 0, 0);
...
internal OS_FileIter *os_file_iter_begin(Arena *arena, String8 path,
  ...
  if (path.size == 0) {
    ...
    w32_iter->handle =
        FindFirstFileW((WCHAR *)path16.str, &w32_iter->find_data);
  }
  ScratchEnd(scratch);
  return iter;
}
...
internal B32 os_file_iter_next(Arena *arena, OS_FileIter *iter,
    ...
    if (!(flags & OS_FileIterFlag_Done) &&
      ...
      do {
        ...
        if (usable_file) {
          ...
          info_out->props.size =
              (U64)w32_iter->find_data.nFileSizeLow |
          ...
          os_w32_dense_time_from_file_time(
  ...
  case 1: {
    ...
    if (result != 0) {
      ...
      info_out->name =
          w32_iter->drive_strings.v[w32_iter->drive_strings_iter_idx];
...
internal U64 os_now_microseconds(void) {
  ...
  if (QueryPerformanceCounter(&large_int_counter)) {
    result = (large_int_counter.QuadPart * Million(1)) /
             os_w32_state.microsecond_resolution;
  }
  return result;
}
...
internal OS_Handle os_process_launch(OS_ProcessLaunchParams *params) {
  ...
  String8 env = {0};
  {
    ...
    if (params->inherit_env != 0) {
      if (all_opts.node_count != 0) {
        ...
        for (String8Node *n = os_w32_state.process_info.environment.first;
             n != 0; n = n->next) {
          Str8ListPush(scratch.arena, &all_opts, n->string);
        }
      } else {
  ...
  if (CreateProcessW(0, (WCHAR *)cmd16.str, 0, 0, inherit_handles,
                     creation_flags, use_null_env_arg ? 0 : (WCHAR *)env16.str,
                     (WCHAR *)dir16.str, &startup_info, &process_info)) {
...
internal OS_Handle os_thread_launch(OS_ThreadFunctionType *func, void *ptr,
  ...
  entity->thread.handle = CreateThread(0, 0, os_w32_thread_entry_point, entity,
                                       0, &entity->thread.tid);
...
internal B32 os_condition_variable_wait(OS_Handle cv, OS_Handle mutex,
  ...
  if (sleep_ms > 0) {
    ...
    result =
        SleepConditionVariableCS(&entity->cv, &mutex_entity->mutex, sleep_ms);
  }
  return result;
}
...
internal B32 os_condition_variable_wait_rw_r(OS_Handle cv, OS_Handle mutex_rw,
  ...
  if (sleep_ms > 0) {
    ...
    result =
        SleepConditionVariableSRW(&entity->cv, &mutex_entity->rw_mutex,
                                  sleep_ms, CONDITION_VARIABLE_LOCKMODE_SHARED);
  }
  return result;
}
...
internal B32 os_condition_variable_wait_rw_w(OS_Handle cv, OS_Handle mutex_rw,
  ...
  if (sleep_ms > 0) {
    OS_W32_Entity *entity = (OS_W32_Entity *)PtrFromInt(cv.u64[0]);
    OS_W32_Entity *mutex_entity = (OS_W32_Entity *)PtrFromInt(mutex_rw.u64[0]);
    result = SleepConditionVariableSRW(&entity->cv, &mutex_entity->rw_mutex,
                                       sleep_ms, 0);
  }
  return result;
}
...
internal OS_Handle os_semaphore_alloc(U32 initial_count, U32 max_count,
  ...
  String16 name16 = Str16From8(scratch.arena, name);
  HANDLE handle =
      CreateSemaphoreW(0, initial_count, max_count, (WCHAR *)name16.str);
...
win32_exception_filter(EXCEPTION_POINTERS *exception_ptrs) {
  ...
  buflen += wnsprintfW(
      buffer + buflen, ArrayCount(buffer) - buflen,
  ...
  if (dbghelp) {
    ...
    PVOID(WINAPI * dbg_SymFunctionTableAccess64)(HANDLE hProcess,
                                                 DWORD64 AddrBase);
    DWORD64(WINAPI * dbg_SymGetModuleBase64)(HANDLE hProcess, DWORD64 qwAddr);
    BOOL(WINAPI * dbg_SymFromAddrW)(HANDLE hProcess, DWORD64 Address,
    ...
    *(FARPROC *)&dbg_SymGetLineFromAddrW64 =
        GetProcAddress(dbghelp, "SymGetLineFromAddrW64");
    *(FARPROC *)&dbg_SymGetModuleInfoW64 =
    ...
    if (dbg_SymSetOptions && dbg_SymInitializeW && dbg_StackWalk64 &&
        ...
        dbg_SymFromAddrW && dbg_SymGetLineFromAddrW64 &&
      ...
      if (dbg_SymInitializeW(process, module_path, TRUE)) {
        ...
        B32 raddbg_pdb_valid = 0;
        {
          ...
          if (dbg_SymGetModuleInfoW64(process, (DWORD64)&win32_exception_filter,
                                      &module)) {
        ...
        if (!raddbg_pdb_valid) {
          buflen += wnsprintfW(
              buffer + buflen, sizeof(buffer) - buflen,
...
#error Arch not supported!
          ...
          for (U32 idx = 0;; idx++) {
            ...
            if (!dbg_StackWalk64(image_type, process, thread, &frame, context,
                                 0, dbg_SymFunctionTableAccess64,
                                 dbg_SymGetModuleBase64, 0)) {
              break;
            }
            ...
            if (dbg_SymFromAddrW(process, address, &displacement,
                                 &symbol.info)) {
              buflen +=
                  wnsprintfW(buffer + buflen, ArrayCount(buffer) - buflen,
              ...
              if (dbg_SymGetLineFromAddrW64(process, address,
                                            &line_displacement, &line)) {
                buflen += wnsprintfW(
                    buffer + buflen, ArrayCount(buffer) - buflen,
                    L", %s line %u", PathFindFileNameW(line.FileName),
                    line.LineNumber);
              }
            } else {
              IMAGEHLP_MODULEW64 module = {0};
  ...
  buflen += wnsprintfW(buffer + buflen, ArrayCount(buffer) - buflen,
                       L"\nVersion: %S%S", BUILD_VERSION_STRING_LITERAL,
  ...
  dialog.dwFlags = TDF_SIZE_TO_CONTENT | TDF_ENABLE_HYPERLINKS |
...
internal void w32_entry_point_caller(int argc, WCHAR **wargv) {
  ...
  // in all SDKs
  {
    ...
    w32_SetThreadDescription_func =
        (W32_SetThreadDescription_Type *)GetProcAddress(module,
  ...
  ArenaParams params = {.reserve_size = MB(1),
                        .commit_size = KB(32),
  ...
  for (int i = 0; i < argc; i += 1) {
    ...
    if (Str8Match(arg8, Str8Lit("--large_pages"), MatchFlag_CaseInsensitive) ||
      ...
      arena_default_reserve_size =
          Max(MB(64), os_w32_state.system_info.large_page_size);
  ...
  }
  {
    ...
    info->initial_path = os_get_current_path(arena);
    {
      ...
      U16 *buffer = PushArrayNoZero(scratch.arena, U16, size);
      if (SUCCEEDED(
              SHGetFolderPathW(0, CSIDL_APPDATA, 0, 0, (WCHAR *)buffer))) {
        info->user_program_data_path =
            str8_from_16(arena, str16_cstring(buffer));
      }
      ScratchEnd(scratch);
    }
    {
      ...
      for (U64 idx = 0;; idx += 1) {
        if (this_proc_env[idx] == 0) {
          if (start_idx == idx) {
            ...
            String16 string16 =
                str16((U16 *)this_proc_env + start_idx, idx - start_idx);
