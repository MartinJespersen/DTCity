...
internal void os_abort(S32 exit_code) { exit(exit_code); }
...
internal B32 os_file_set_times(OS_Handle file, DateTime date_time) {
  if (os_handle_match(file, os_handle_zero())) {
    return 0;
  }
  int fd = (int)file.u64[0];
...
internal B32 os_process_join(OS_Handle handle, U64 endt_us) { NotImplemented; }

internal void os_process_detach(OS_Handle handle) { NotImplemented; }
...
internal void os_rw_mutex_release(OS_Handle rw_mutex) {
  if (os_handle_match(rw_mutex, os_handle_zero())) {
    return;
  }
...
internal void os_condition_variable_release(OS_Handle cv) {
  if (os_handle_match(cv, os_handle_zero())) {
    return;
  }
...
internal B32 os_condition_variable_wait_rw_r(OS_Handle cv, OS_Handle mutex_rw,
  ...
  if (os_handle_match(mutex_rw, os_handle_zero())) {
    return 0;
  }
  OS_LNX_Entity *cv_entity = (OS_LNX_Entity *)cv.u64[0];
...
internal OS_Handle os_semaphore_open(String8 name) { NotImplemented; }

internal void os_semaphore_close(OS_Handle semaphore) { NotImplemented; }
