...
{
    U128 hash = HashU128FromStr8(string);
    return hash;
}
...
{
    (void)seed;
    U128 result = HashU128FromStr8(string);
    return result.u64[0];
}
...
fnt_handle_from_tag(FNT_Tag tag)
{
    ...
    {
        for (FNT_FontHashNode* n = fnt_state->font_hash_table[slot_idx].first; n != 0;
             n = n->hash_next)
        {
...
fnt_tag_from_path(String8 path)
{
    ...
    if (existing_node == 0)
    {
        ...
        existing_node = PushArray(fnt_state->permanent_arena, FNT_FontHashNode, 1);
        existing_node->tag = result;
        existing_node->handle = handle;
        existing_node->metrics = fp_metrics_from_font(existing_node->handle);
        existing_node->path = PushStr8Copy(fnt_state->permanent_arena, path);
...
fnt_atlas_region_alloc(Arena* arena, FNT_Atlas* atlas, Vec2S16 needed_size)
{
    ...
    FNT_AtlasRegionNode* node = 0;
    {
        ...
        for (FNT_AtlasRegionNode *n = atlas->root, *next = 0; n != 0; n = next, next = 0)
        {
            ...
            // rjf: calculate if this node can be allocated (all children are
            // non-allocated)
            ...
            {
                for (Corner corner = (Corner)0; corner < Corner_COUNT;
                     corner = (Corner)(corner + 1))
                {
                    if (n->children[corner] == 0)
                    {
                        n->children[corner] = PushArray(arena, FNT_AtlasRegionNode, 1);
                        ...
                        n->children[corner]->max_free_size[Corner_00] =
                            n->children[corner]->max_free_size[Corner_01] =
                                n->children[corner]->max_free_size[Corner_10] =
                                    n->children[corner]->max_free_size[Corner_11] =
                                        v2s16(child_size.x / 2, child_size.y / 2);
                    }
        ...
        for (FNT_AtlasRegionNode* p = node->parent; p != 0; p = p->parent)
        {
            ...
            if (parent != 0)
            {
                ...
                parent->max_free_size[p_corner].x =
                    Max(Max(p->max_free_size[Corner_00].x, p->max_free_size[Corner_01].x),
                        Max(p->max_free_size[Corner_10].x, p->max_free_size[Corner_11].x));
                parent->max_free_size[p_corner].y =
                    Max(Max(p->max_free_size[Corner_00].y, p->max_free_size[Corner_01].y),
                        Max(p->max_free_size[Corner_10].y, p->max_free_size[Corner_11].y));
            }
        }
    }
...
fnt_atlas_region_release(FNT_Atlas* atlas, Rng2S16 region)
{
    ...
    Corner node_corner = Corner_Invalid;
    {
        ...
        {
            // rjf: is the region within this node's boundaries? (either this node, or
            // a descendant)
            if (n_p0.x <= region.p0.x && region.p0.x < n_p0.x + n_sz.x && n_p0.y <= region.p0.y &&
                region.p0.y < n_p0.y + n_sz.y)
            {
                // rjf: check the region against this node
                if (region.p0.x == n_p0.x && region.p0.y == n_p0.y && region_size.x == n_sz.x &&
                    region_size.y == n_sz.y)
                {
                    node = n;
                ...
                {
                    Vec2S16 r_midpoint =
                        v2s16(region.p0.x + region_size.x / 2, region.p0.y + region_size.y / 2);
                    Vec2S16 n_midpoint = v2s16(n_p0.x + n_sz.x / 2, n_p0.y + n_sz.y / 2);
...
fnt_piece_chunk_list_push_new(Arena* arena, FNT_PieceChunkList* list, U64 cap)
{
    ...
    {
        node = PushArray(arena, FNT_PieceChunkNode, 1);
        node->v = PushArrayNoZero(arena, FNT_Piece, cap);
        node->cap = cap;
...
fnt_piece_array_from_chunk_list(Arena* arena, FNT_PieceChunkList* list)
{
    ...
    array.v = PushArrayNoZero(arena, FNT_Piece, array.count);
    U64 write_idx = 0;
...
fnt_piece_array_copy(Arena* arena, FNT_PieceArray* src)
{
    ...
    dst.count = src->count;
    dst.v = PushArrayNoZero(arena, FNT_Piece, dst.count);
...
fnt_hash2style_from_tag_size_flags(FNT_Tag tag, F32 size, FNT_RasterFlags flags)
{
    ...
    FNT_Hash2StyleRasterCacheNode* hash2style_node = 0;
    {
        ...
        for (FNT_Hash2StyleRasterCacheNode* n = slot->first; n != 0; n = n->hash_next)
        {
        ...
        if (Unlikely(hash2style_node == 0))
        {
            ...
            hash2style_node = PushArray(fnt_state->raster_arena, FNT_Hash2StyleRasterCacheNode, 1);
            ...
            hash2style_node->utf8_class1_direct_map =
                PushArrayNoZero(fnt_state->raster_arena, FNT_RasterCacheInfo, 256);
            hash2style_node->hash2info_slots_count = 1024;
            hash2style_node->hash2info_slots =
                PushArray(fnt_state->raster_arena, FNT_Hash2InfoRasterCacheSlot,
...
static FNT_Run
fnt_run_from_string(FNT_Tag tag, F32 size, F32 base_align_px, F32 tab_size_px,
                    FNT_RasterFlags flags, String8 string)
{
    //- rjf: map tag/size to style node
    FNT_Hash2StyleRasterCacheNode* hash2style_node =
        fnt_hash2style_from_tag_size_flags(tag, size, flags);
    ...
    if (hash2style_node->run_slots_frame_index != fnt_state->frame_index)
    {
        ...
        hash2style_node->run_slots =
            PushArray(fnt_state->frame_arena, FNT_RunCacheSlot, hash2style_node->run_slots_count);
    ...
    FNT_RunCacheNode* run_node = 0;
    {
        ...
        {
            if (Str8Match(n->string, string, 0))
            {
                run_node = n;
                break;
            }
    ...
    else
    {
        ...
        {
            //- rjf: decode next codepoint & get piece substring, or continuation
            // rule
            ...
            //- rjf: do not need another codepoint? -> grab substring, bump piece
            // start idx
            String8 piece_substring =
                str8_substr(string, r1u64(piece_substring_start_idx, piece_substring_end_idx));
            piece_substring_start_idx = idx;
            piece_substring_end_idx = idx;

            //- rjf: determine if this piece is a tab - if so, use space info to
            // draw
            B32 is_tab = (piece_substring.size == 1 && piece_substring.str[0] == '\t');
            if (is_tab)
            {
                run_is_cacheable = 0;
                piece_substring = Str8Lit(" ");
            }
            ...
            U64 piece_hash = 0;
            {
                ...
                if (piece_substring.size == 1 &&
                    hash2style_node->utf8_class1_direct_map_mask[piece_substring.str[0] / 64] &
                        (1ull << (piece_substring.str[0] % 64)))
                {
                ...
                if (piece_substring.size > 1)
                {
                    ...
                    FNT_Hash2InfoRasterCacheSlot* slot =
                        &hash2style_node->hash2info_slots[slot_idx];
                    for (FNT_Hash2InfoRasterCacheNode* node = slot->first; node != 0;
                         node = node->hash_next)
                    {
            ...
            if (info == 0)
            {
                ...
                if (font_handle_mapped_on_miss == 0)
                {
                    ...
                    FNT_FontHashNode* existing_node = 0;
                    {
                        for (FNT_FontHashNode* n = fnt_state->font_hash_table[font_slot_idx].first;
                             n != 0; n = n->hash_next)
                        {
                ...
                if (size > 0)
                {
                    ...
                    if (flags & FNT_RasterFlag_Smooth)
                    {
                        fp_flags |= FP_RasterFlag_Smooth;
                    }
                    if (flags & FNT_RasterFlag_Hinted)
                    {
                        fp_flags |= FP_RasterFlag_Hinted;
                    }
                    raster = fp_raster(scratch.arena, font_handle, FloorF32(size), flags,
                                       piece_substring);
                }
                ...
                {
                    U64 num_atlases = 0;
                    for (FNT_Atlas* atlas = fnt_state->first_atlas;;
                         atlas = atlas->next, num_atlases += 1)
                    {
                        // rjf: create atlas if needed
                        if (atlas == 0 && num_atlases < 64)
                        {
                            atlas = PushArray(fnt_state->raster_arena, FNT_Atlas, 1);
                            DLLPushBack(fnt_state->first_atlas, fnt_state->last_atlas, atlas);
                            atlas->root_dim = v2s16(1024, 1024);
                            atlas->root =
                                PushArray(fnt_state->raster_arena, FNT_AtlasRegionNode, 1);
                            atlas->root->max_free_size[Corner_00] =
                                atlas->root->max_free_size[Corner_01] =
                                    atlas->root->max_free_size[Corner_10] =
                                        atlas->root->max_free_size[Corner_11] =
                                            v2s16(atlas->root_dim.x / 2, atlas->root_dim.y / 2);
                            atlas->texture =
                                R_Tex2dAlloc(R_ResourceKind_Dynamic,
                                             v2s32((S32)atlas->root_dim.x, (S32)atlas->root_dim.y),
                                             R_Tex2DFormat_RGBA8, 0);
                        }

                        // rjf: allocate from atlas
                        if (atlas != 0)
                        {
                            Vec2S16 needed_dimensions =
                                v2s16(raster.atlas_dim.x + 2, raster.atlas_dim.y + 2);
                            chosen_atlas_region = fnt_atlas_region_alloc(fnt_state->raster_arena,
                                                                         atlas, needed_dimensions);
                ...
                // rjf: upload rasterization to allocated region of atlas texture
                // memory
                if (chosen_atlas != 0)
                {
                    Rng2S32 subregion = {chosen_atlas_region.x0, chosen_atlas_region.y0,
                                         chosen_atlas_region.x0 + raster.atlas_dim.x,
                                         chosen_atlas_region.y0 + raster.atlas_dim.y};
                    R_FillTex2dRegion(chosen_atlas->texture, subregion, raster.atlas);
                }
                ...
                // rjf: allocate & fill & push node
                {
                    ...
                    else
                    {
                        ...
                        FNT_Hash2InfoRasterCacheSlot* slot =
                            &hash2style_node->hash2info_slots[slot_idx];
                        FNT_Hash2InfoRasterCacheNode* node = PushArrayNoZero(
                            fnt_state->raster_arena, FNT_Hash2InfoRasterCacheNode, 1);
            ...
            if (info != 0)
            {
                ...
                if (is_tab)
                {
                    advance = FloorF32(tab_size_px) -
                              mod_f32(FloorF32(base_align_px), FloorF32(tab_size_px));
                }

                // rjf: push piece
                {
                    FNT_Piece* piece = fnt_piece_chunk_list_push_new(fnt_state->frame_arena,
                                                                     &piece_chunks, string.size);
                    {
                        piece->texture = atlas ? atlas->texture : R_HandleZero();
                        piece->subrect = r2s16p(info->subrect.x0, info->subrect.y0,
                        ...
                        piece->offset =
                            v2s16(0, -(hash2style_node->ascent + hash2style_node->descent));
                    }
    ...
    if (run_is_cacheable)
    {
        run_node = PushArray(fnt_state->frame_arena, FNT_RunCacheNode, 1);
        SLLQueuePush(run_slot->first, run_slot->last, run_node);
        run_node->string = PushStr8Copy(fnt_state->frame_arena, string);
        run_node->run = run;
    }
...
static String8List
fnt_wrapped_string_lines_from_font_size_string_max(Arena* arena, FNT_Tag font, F32 size,
                                                   F32 base_align_px, F32 tab_size_px,
                                                   String8 string, F32 max)
{
    String8List list = {0};
    {
        ...
        for (FNT_Piece *piece = pieces_first, *next = 0; piece != 0 && piece <= pieces_opl;
             piece = next)
        {
            if (piece != 0)
            {
            ...
            {
                String8 line =
                    Str8(string.str + line_start_off_bytes, off_bytes - line_start_off_bytes);
            ...
            {
                String8 line = Str8(string.str + line_start_off_bytes,
                                    line_end_off_bytes - line_start_off_bytes);
                line = Str8SkipChopWhitespace(line);
                if (line.size != 0)
                {
                    Str8ListPush(arena, &list, line);
...
static Vec2F32
fnt_dim_from_tag_size_string(FNT_Tag tag, F32 size, F32 base_align_px, F32 tab_size_px,
                             String8 string)
{
    Temp scratch = ScratchBegin(0, 0);
    Vec2F32 result = {0};
...
static Vec2F32
fnt_dim_from_tag_size_string_list(FNT_Tag tag, F32 size, F32 base_align_px, F32 tab_size_px,
                                  String8List list)
{
    Vec2F32 sum = {0};
    ...
    {
        Vec2F32 str_dim =
            fnt_dim_from_tag_size_string(tag, size, base_align_px, tab_size_px, n->string);
        sum.x += str_dim.x;
...
{
    F32 result = fnt_dim_from_tag_size_string(tag, size, 0, 0, Str8Lit("H")).x;
    return result;
}

static U64
fnt_char_pos_from_tag_size_string_p(FNT_Tag tag, F32 size, F32 base_align_px, F32 tab_size_px,
                                    String8 string, F32 p)
{
    Temp scratch = ScratchBegin(0, 0);
    ...
    {
        F32 this_piece_offset_px = AbsF32(offset_px - p);
...
fnt_metrics_from_tag_size(FNT_Tag tag, F32 size)
{
    ...
    {
        result.ascent = FloorF32(size) * metrics.ascent / metrics.design_units_per_em;
        result.descent = FloorF32(size) * metrics.descent / metrics.design_units_per_em;
        result.line_gap = FloorF32(size) * metrics.line_gap / metrics.design_units_per_em;
        result.capital_height =
            FloorF32(size) * metrics.capital_height / metrics.design_units_per_em;
    }
    return result;
}
...
fnt_init(void)
{
    ...
    fnt_state = PushArray(arena, FNT_State, 1);
    fnt_state->permanent_arena = arena;
    fnt_state->raster_arena = ArenaAlloc();
    fnt_state->frame_arena = ArenaAlloc();
    fnt_state->font_hash_table_size = 64;
    fnt_state->font_hash_table =
        PushArray(fnt_state->permanent_arena, FNT_FontHashSlot, fnt_state->font_hash_table_size);
    fnt_reset();
}
...
fnt_reset(void)
{
    ...
    {
        R_Tex2dRelease(a->texture);
    }
    fnt_state->first_atlas = fnt_state->last_atlas = 0;
    ArenaClear(fnt_state->raster_arena);
    fnt_state->hash2style_slots_count = 1024;
    fnt_state->hash2style_slots = PushArray(fnt_state->raster_arena, FNT_Hash2StyleRasterCacheSlot,
...
fnt_frame(void)
{
    ...
    ArenaClear(fnt_state->frame_arena);
}
