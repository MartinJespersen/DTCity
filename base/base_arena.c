...
arena_alloc_(ArenaParams* params)
{
    ...
    if (Unlikely(base == 0))
    {
        os_graphical_message(1, str8_lit("Fatal Allocation Failure"),
...
arena_push(Arena* arena, U64 size, U64 align)
{
    ...
    if (current->res < pos_pst && !(arena->flags & ArenaFlag_NoChain))
    {
        ...
        if (new_block == 0)
        {
            ...
            ArenaParams params = {
                .reserve_size = res_size, .commit_size = cmt_size, .flags = current->flags};
...
arena_pop_to(Arena* arena, U64 pos)
{
    ...
    for (Arena* prev = 0; current->base_pos >= big_pos; current = prev)
    {
        ...
        AsanPoisonMemoryRegion((U8*)current + ARENA_HEADER_SIZE,
                               current->res_size - ARENA_HEADER_SIZE);
    }
#else
    for (Arena* prev = 0; current->base_pos >= big_pos; current = prev)
    {
