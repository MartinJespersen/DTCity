...
layout(location = 0) in uint road_id[];
...
void main() {
    uint node_0_id = road_id[1];
    uint node_1_id = road_id[2];

    if (node_0_id == node_1_id) {
        vec2 node_0 = gl_in[node_0_id].gl_Position.xy;
        vec2 node_1 = gl_in[node_1_id].gl_Position.xy;
