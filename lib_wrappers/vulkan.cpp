...
// mgj: Road
static void
RoadDescriptorCreate(VkDescriptorPool desc_pool, RoadDescriptorCreateInfo* info,
                     VK_Texture* texture)
{
...
RoadTextureCreate(U32 thread_id, AssetManagerCommandPool threaded_cmd_pool,
                  RoadTextureCreateInput* input, VK_Texture* texture)
{
...
static void
RoadDescriptorSetCreate(VkDevice device, VkDescriptorPool desc_pool, VK_Texture* texture,
                        Road* road, U32 frames_in_flight)
{
    ScratchScope scratch = ScratchScope(0, 0);
                       ...
                       String8 texture_path, CgltfSampler sampler)
{
    ...
    VkSamplerCreateInfo sampler_create_info = VkSamplerFromCgltfSampler(sampler);
...
static void
CarPipelineCreate(VK_Texture* texture, Car* car)
{
    VulkanContext* vk_ctx = VulkanCtxGet();
          ...
          Buffer<city::CarVertex> vertex_buffer, Buffer<U32> index_buffer)
{
    ...
    CarTextureCreate(texture_id, texture_path, sampler);
...
CarDestroy(VulkanContext* vk_ctx, Car* car)
{
    ...
    if (asset_item->is_loaded)
    {
        ...
        VK_TextureDestroy(vk_ctx, &asset_item->item);
    }
...
CarUpdate(city::CarSim* car_sim, Buffer<city::CarInstance> instance_buffer, U32 image_idx)
{
    ...
    AssetItem<Texture>* asset_item = AssetManagerTextureGetSlot(w_car->texture_id);

    if (!w_car->is_pipeline_created && asset_item->is_loaded)
    {
        CarPipelineCreate(&asset_item->item, w_car);
...
AssetManagerCmdDoneCheck()
{
         ...
         cmd_queue_item = cmd_queue_item->next)
    {
        ...
        if (result == VK_SUCCESS)
        {
            ...
            {
                AssetItem<Texture>* asset = AssetManagerTextureGetSlot(cmd_queue_item->asset_id);
