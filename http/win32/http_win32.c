...
HTTP_Init(void)
{
    if (http_w32_state == 0)
    {
        ...
        http_w32_state->hSession = WinHttpOpen(0, WINHTTP_ACCESS_TYPE_NO_PROXY,
...
HTTP_Request(Arena* arena, String8 host, String8 path, String8 body, HTTP_RequestParams* params)
{
    ...
    String8List response_data_strings = {0};
    {
        ...
        String8 url_path_part = {0};
        {
            ...
            U64 first_non_protocol_slash_pos = FindSubstr8(url, Str8Lit("/"), post_protocol_pos, 0);
        ...
        String16 hostname16 = {0};
        {
            ...
            else if (Str8Match(url_protocol_part, Str8Lit("https://"), MatchFlag_CaseInsensitive))
            {
                port = INTERNET_DEFAULT_HTTPS_PORT;
            }
            else if (Str8Match(url_protocol_part, Str8Lit("http://"), MatchFlag_CaseInsensitive))
            {
                port = INTERNET_DEFAULT_HTTP_PORT;
            }
            else if (Str8Match(url_protocol_part, Str8Lit("ftp://"), MatchFlag_CaseInsensitive))
            {
                port = 21;
            }
            else if (Str8Match(url_protocol_part, Str8Lit("ssh://"), MatchFlag_CaseInsensitive))
            {
                port = 22;
            }
        ...
        if (good)
        {
            hRequest = WinHttpOpenRequest(hConnect, verb, path_name, 0, WINHTTP_NO_REFERER,
        ...
        if (good)
        {
            good = !!WinHttpSendRequest(hRequest, header, header_size, optional, optional_size,
                                        optional_size, 0);
        }
