...
OS_FileRead(OS_Handle file, Rng1U64 rng, void* out_data)
{
    ...
    {
        int read_result = pread(fd, (U8*)out_data + total_num_bytes_read,
...
os_file_iter_next(Arena* arena, OS_FileIter* iter, OS_FileInfo* info_out)
{
    ...
    for (;;)
    {
        ...
        if (good)
        {
            Temp scratch = ScratchBegin(&arena, 1);
            String8 full_path =
                push_str8f(scratch.arena, "%s/%s", lnx_iter->path.str, lnx_iter->dp->d_name);
...
OS_ThreadLaunch(OS_ThreadFunctionType* func, void* ptr, void* params)
{
    ...
    {
        int pthread_result =
            pthread_create(&entity->thread.handle, 0, os_lnx_thread_entry_point, entity);
        if (pthread_result == -1)
        {
...
os_condition_variable_wait(OS_Handle cv, OS_Handle mutex, U64 endt_us)
{
    ...
    int wait_result = pthread_cond_timedwait(&cv_entity->cv.cond_handle,
                                             &mutex_entity->mutex_handle, &endt_timespec);
...
os_condition_variable_wait_rw_r(OS_Handle cv, OS_Handle mutex_rw, U64 endt_us)
{
    ...
    for (;;)
    {
        ...
        int wait_result = pthread_cond_timedwait(
            &cv_entity->cv.cond_handle, &cv_entity->cv.rwlock_mutex_handle, &endt_timespec);
...
OS_SemaphoreAlloc(U32 initial_count, U32 max_count, String8 name)
{
    ...
    {
        sem_t* s =
            (sem_t*)mmap(0, sizeof(*s), PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
...
OS_SystemTimerRead()
{
    ...
    U64 Result = OS_CpuTimerRead() * (U64)Value.tv_sec + (U64)Value.tv_usec;
    return Result;
}
////////////////////////////////
    ...
    //- rjf: set up OS layer
    {
        ...
        //- rjf: grab dynamically allocated system info
        {
            ...
            if (got_final_result && size > 0)
            {
                ...
                info->machine_name.str =
                    PushArrayNoZero(os_lnx_state.arena, U8, info->machine_name.size + 1);
