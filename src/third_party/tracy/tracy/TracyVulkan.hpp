...
class VkCtx
{
  ...
  public:
#if defined TRACY_VK_USE_SYMBOL_TABLE
    VkCtx(VkInstance instance, VkPhysicalDevice physdev, VkDevice device, VkQueue queue,
          VkCommandBuffer cmdbuf, PFN_vkGetInstanceProcAddr instanceProcAddr,
          PFN_vkGetDeviceProcAddr deviceProcAddr, bool calibrated)
#else
    VkCtx(VkPhysicalDevice physdev, VkDevice device, VkQueue queue, VkCommandBuffer cmdbuf,
        ...
        : m_device(device), m_timeDomain(VK_TIME_DOMAIN_DEVICE_EXT),
          m_context(GetGpuCtxCounter().fetch_add(1, std::memory_order_relaxed)), m_head(0),
          m_tail(0), m_oldCnt(0), m_queryCount(QueryCount)
#if !defined TRACY_VK_USE_SYMBOL_TABLE
          ,
...
#endif
    {
            ...
            m_vkGetCalibratedTimestampsEXT)
        {
            FindAvailableTimeDomains(
                physdev, VK_FUNCTION_WRAPPER(vkGetPhysicalDeviceCalibrateableTimeDomainsEXT));
        }
        ...
        if (m_timeDomain == VK_TIME_DOMAIN_DEVICE_EXT)
        {
            ...
            VK_FUNCTION_WRAPPER(
                vkGetQueryPoolResults(device, m_query, 0, 1, sizeof(tgpu), &tgpu, sizeof(tgpu),
     ...
     * VK_EXT_host_query_reset (core with 1.2 and non-optional) and VK_EXT_calibrated_timestamps.
     * This requires the physical device to have another time domain apart from DEVICE to be
     * calibrateable.
     */
#if defined TRACY_VK_USE_SYMBOL_TABLE
    VkCtx(VkInstance instance, VkPhysicalDevice physdev, VkDevice device,
          PFN_vkGetInstanceProcAddr instanceProcAddr, PFN_vkGetDeviceProcAddr deviceProcAddr)
#else
    ...
    void
    Name(const char* name, uint16_t len)
    {
    ...
    void
    Collect(VkCommandBuffer cmdbuf)
    {
        ...
        VK_FUNCTION_WRAPPER(vkGetQueryPoolResults(
            m_device, m_query, wrappedTail, cnt, sizeof(int64_t) * m_queryCount * 2, m_res,
            sizeof(int64_t) * 2, VK_QUERY_RESULT_64_BIT | VK_QUERY_RESULT_WITH_AVAILABILITY_BIT));
    ...
    tracy_force_inline unsigned int
    NextQueryId()
    {
    ...
    tracy_force_inline uint8_t
    GetId() const
    {
        return m_context;
    }

    tracy_force_inline VkQueryPool
    GetQueryPool() const
    {
        return m_query;
    }
  ...
  private:
    ...
    Calibrate(VkDevice device, int64_t& tCpu, int64_t& tGpu)
    {
    ...
    CreateQueryPool()
    {
        ...
        while (VK_FUNCTION_WRAPPER(vkCreateQueryPool(m_device, &poolInfo, nullptr, &m_query) !=
                                   VK_SUCCESS))
        {
            m_queryCount /= 2;
            poolInfo.queryCount = m_queryCount;
    ...
    tracy_force_inline void
    FindAvailableTimeDomains(VkPhysicalDevice physicalDevice,
                             PFN_vkGetPhysicalDeviceCalibrateableTimeDomainsEXT
    ...
    tracy_force_inline void
    WriteInitialItem(VkPhysicalDevice physdev, int64_t tcpu, int64_t tgpu)
    {
        uint8_t flags = 0;
        if (m_timeDomain != VK_TIME_DOMAIN_DEVICE_EXT)
...
#if defined TRACY_VK_USE_SYMBOL_TABLE
    void
    PopulateSymbolTable(VkInstance instance, PFN_vkGetInstanceProcAddr instanceProcAddr,
...
class VkCtxScope
{
  public:
...
#endif
    {
        ...
        CONTEXT_VK_FUNCTION_WRAPPER(vkCmdWriteTimestamp(
            cmdbuf, VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT, ctx->m_query, queryId));
    ...
    tracy_force_inline
    VkCtxScope(VkCtx* ctx, const SourceLocationData* srcloc, VkCommandBuffer cmdbuf, int32_t depth,
               bool is_active)
#ifdef TRACY_ON_DEMAND
        : m_active(is_active && GetProfiler().IsConnected())
#else
    ...
    tracy_force_inline
    VkCtxScope(VkCtx* ctx, uint32_t line, const char* source, size_t sourceSz, const char* function,
...
#endif
    {
        ...
        const auto srcloc = Profiler::AllocSourceLocation(line, source, sourceSz, function,
    ...
    tracy_force_inline ~VkCtxScope()
    {
        ...
        CONTEXT_VK_FUNCTION_WRAPPER(vkCmdWriteTimestamp(
            m_cmdbuf, VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT, m_ctx->m_query, queryId));
...
#if defined TRACY_VK_USE_SYMBOL_TABLE
static inline VkCtx*
CreateVkContext(VkInstance instance, VkPhysicalDevice physdev, VkDevice device, VkQueue queue,
                VkCommandBuffer cmdbuf, PFN_vkGetInstanceProcAddr instanceProcAddr,
                PFN_vkGetDeviceProcAddr getDeviceProcAddr, bool calibrated = false)
#else
static inline VkCtx*
CreateVkContext(VkPhysicalDevice physdev, VkDevice device, VkQueue queue, VkCommandBuffer cmdbuf,
...
#if defined TRACY_VK_USE_SYMBOL_TABLE
    new (ctx) VkCtx(instance, physdev, device, queue, cmdbuf, instanceProcAddr, getDeviceProcAddr,
                    calibrated);
#else
...
#if defined TRACY_VK_USE_SYMBOL_TABLE
static inline VkCtx*
CreateVkContext(VkInstance instance, VkPhysicalDevice physdev, VkDevice device,
                PFN_vkGetInstanceProcAddr instanceProcAddr,
                PFN_vkGetDeviceProcAddr getDeviceProcAddr)
#else
static inline VkCtx*
CreateVkContext(VkPhysicalDevice physdev, VkDevice device, PFN_vkResetQueryPoolEXT qpreset,
                PFN_vkGetPhysicalDeviceCalibrateableTimeDomainsEXT gpdctd,
                PFN_vkGetCalibratedTimestampsEXT gct)
#endif
{
...
#define TracyVkContext(instance, physdev, device, queue, cmdbuf, instanceProcAddr, deviceProcAddr) \
    tracy::CreateVkContext(instance, physdev, device, queue, cmdbuf, instanceProcAddr,             \
                           deviceProcAddr);
#else
...
#endif
#if defined TRACY_VK_USE_SYMBOL_TABLE
#define TracyVkContextCalibrated(instance, physdev, device, queue, cmdbuf, instanceProcAddr,       \
                                 deviceProcAddr)                                                   \
    tracy::CreateVkContext(instance, physdev, device, queue, cmdbuf, instanceProcAddr,             \
                           deviceProcAddr, true);
#else
...
#define TracyVkNamedZone(ctx, varname, cmdbuf, name, active)                                       \
    static constexpr tracy::SourceLocationData TracyConcat(                                        \
        __tracy_gpu_source_location, TracyLine){name, TracyFunction, TracyFile,                    \
