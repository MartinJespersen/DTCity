...
ImGui_ImplVulkan_DestroyWindowRenderBuffers(VkDevice device,
                                            ...
                                            const VkAllocationCallbacks* allocator);
void
ImGui_ImplVulkanH_DestroyFrame(VkDevice device, ImGui_ImplVulkanH_Frame* fd,
                               const VkAllocationCallbacks* allocator);
void
ImGui_ImplVulkanH_DestroyFrameSemaphores(VkDevice device, ImGui_ImplVulkanH_FrameSemaphores* fsd,
                                         const VkAllocationCallbacks* allocator);
void
ImGui_ImplVulkanH_CreateWindowSwapChain(VkPhysicalDevice physical_device, VkDevice device,
                                        ImGui_ImplVulkanH_Window* wd,
                                        const VkAllocationCallbacks* allocator, int w, int h,
                                        uint32_t min_image_count, VkImageUsageFlags image_usage);
void
ImGui_ImplVulkanH_CreateWindowCommandBuffers(VkPhysicalDevice physical_device, VkDevice device,
...
static void
CreateOrResizeBuffer(VkBuffer& buffer, VkDeviceMemory& buffer_memory, VkDeviceSize& buffer_size,
...
static void
ImGui_ImplVulkan_DestroyTexture(ImTextureData* tex)
{
...
void
ImGui_ImplVulkan_UpdateTexture(ImTextureData* tex)
{
    if (tex->Status == ImTextureStatus_OK)
        return;
    ImGui_ImplVulkan_Data* bd = ImGui_ImplVulkan_GetBackendData();
    ...
    if (tex->Status == ImTextureStatus_WantCreate || tex->Status == ImTextureStatus_WantUpdates)
    {
        ...
        // tex->Updates[] to upload individual regions. We could use the smaller rect on _WantCreate
        // but using the full rect allows us to clear the texture.
        ...
        // Copy to Image:
        {
            ...
            vkCmdPipelineBarrier(bd->TexCommandBuffer,
                                 ...
                                 VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, nullptr, 1, upload_barrier,
                                 1, copy_barrier);
...
static VkPipeline
ImGui_ImplVulkan_CreatePipeline(VkDevice device, const VkAllocationCallbacks* allocator,
...
void
ImGui_ImplVulkan_CreateMainPipeline(const ImGui_ImplVulkan_PipelineInfo* pipeline_info_in)
{
    ImGui_ImplVulkan_Data* bd = ImGui_ImplVulkan_GetBackendData();
    ImGui_ImplVulkan_InitInfo* v = &bd->VulkanInitInfo;
    if (bd->Pipeline)
...
void
ImGui_ImplVulkanH_CreateOrResizeWindow(VkInstance instance, VkPhysicalDevice physical_device,
...
void
ImGui_ImplVulkanH_DestroyWindow(VkInstance instance, VkDevice device, ImGui_ImplVulkanH_Window* wd,
                                const VkAllocationCallbacks* allocator)
{
