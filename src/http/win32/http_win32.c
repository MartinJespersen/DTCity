...
HTTP_Init()
{
    if (http_w32_state == 0)
    {
        ...
        http_w32_state->hSession = WinHttpOpen(0, WINHTTP_ACCESS_TYPE_NO_PROXY, WINHTTP_NO_PROXY_NAME, WINHTTP_NO_PROXY_BYPASS, 0);
    }
}
...
HTTP_Request(Arena* arena, String8 host, String8 path, String8 body, HTTP_RequestParams* params)
{
    ...
    String8List response_data_strings = {0};
    {
        ...
        String16 hostname16 = {0};
        {
            ...
            else if (str8_match(url_protocol_part, str8_lit("https://"), MatchFlag_CaseInsensitive))
            {
                port = INTERNET_DEFAULT_HTTPS_PORT;
            }
            else if (str8_match(url_protocol_part, str8_lit("http://"), MatchFlag_CaseInsensitive))
            {
                port = INTERNET_DEFAULT_HTTP_PORT;
            }
            else if (str8_match(url_protocol_part, str8_lit("ftp://"), MatchFlag_CaseInsensitive))
            {
                port = 21;
            }
            else if (str8_match(url_protocol_part, str8_lit("ssh://"), MatchFlag_CaseInsensitive))
            {
                port = 22;
            }
        ...
        {
            hConnect = WinHttpConnect(http_w32_state->hSession, (WCHAR*)hostname16.str, port, 0);
            good = (hConnect != 0);
        }
        ...
        {
            DWORD flags = 0;
            if (str8_match(url_protocol_part, str8_lit("https://"), MatchFlag_CaseInsensitive))
            {
                flags = WINHTTP_FLAG_SECURE;
            }
            hRequest = WinHttpOpenRequest(hConnect, verb, path_name, 0, WINHTTP_NO_REFERER, WINHTTP_DEFAULT_ACCEPT_TYPES, flags);
            good = (hRequest != 0);
        }
        ...
        if (good)
        {
            good = !!WinHttpSendRequest(hRequest, header, header_size, optional, optional_size, optional_size, 0);
        }
        ...
        WinHttpQueryHeaders(hRequest, WINHTTP_QUERY_STATUS_CODE | WINHTTP_QUERY_FLAG_NUMBER, WINHTTP_HEADER_NAME_BY_INDEX, &status_code, &status_code_size, WINHTTP_NO_HEADER_INDEX);
        code = status_code;
