...
lib_internal void TCTX_InitAndEquip(TCTX *tctx) {
...
lib_internal void TCTX_Release() {
...
lib_internal TCTX *TCTX_Get() { return (tctx_thread_local); }

lib_internal Arena *TCTX_ScratchGet(Arena **conflicts, U64 count) {
  TCTX *tctx = TCTX_Get();
...
lib_internal void tctx_set_thread_name(String8 string) {
  TCTX *tctx = TCTX_Get();
...
lib_internal String8 tctx_get_thread_name() {
  TCTX *tctx = TCTX_Get();
  String8 result = str8(tctx->thread_name, tctx->thread_name_size);
  return (result);
}

lib_internal void tctx_write_srcloc(char *file_name, U64 line_number) {
  TCTX *tctx = TCTX_Get();
  tctx->file_name = file_name;
  tctx->line_number = line_number;
}

lib_internal void tctx_read_srcloc(char **file_name, U64 *line_number) {
  TCTX *tctx = TCTX_Get();
...
lib_internal void LogRelease(Log *log) { arena_release(log->arena); }

////////////////////////////////
//~ mgj: Log Building/Clearing

lib_internal void LogMsg(LogMsgKind kind, String8 string) {
  if (tctx_thread_local != 0 && tctx_thread_local->log != 0 &&
      tctx_thread_local->log->top_scope != 0) {
    String8 string_copy = push_str8_copy(tctx_thread_local->log->arena, string);
    str8_list_push(tctx_thread_local->log->arena,
                   &tctx_thread_local->log->top_scope->strings[kind],
                   string_copy);
  }
}

lib_internal void LogMsgF(LogMsgKind kind, char *fmt, ...) {
...
lib_internal void LogScopeBegin() {
  if (tctx_thread_local != 0 && tctx_thread_local->log) {
...
lib_internal LogScopeResult LogScopeEnd(Arena *arena) {
