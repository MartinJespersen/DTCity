...
fp_init(void)
{
    ...
    fp_ft_state = PushStruct(arena, FP_FT_State);
...
fp_raster(Arena* arena, FP_Handle handle, F32 size, FP_RasterFlags flags, String8 string)
{
    ...
    if (font.face != 0)
    {
        ...
        //- rjf: unpack string
        String32 string32 = Str32From8(scratch.arena, string);

        //- rjf: measure
        S16 total_width = 0;
        for (U32 idx = 0; idx < string32.size; idx++)
        {
        ...
        Vec2S16 dim = {(S16)total_width + 1, (S16)height + 1};
        U64 atlas_size = dim.x * dim.y * 4;
        U8* atlas = PushArray(arena, U8, atlas_size);
        S32 baseline = ascent;
        S32 atlas_write_x = 0;
        for (U32 idx = 0; idx < string32.size; idx++)
        {
