...
static void TextureDestroy(VulkanContext *vk_ctx, Texture *texture) {
...
// queue family
VkFormat VK_SupportedFormat(VkPhysicalDevice physical_device,
                            VkFormat *candidates, U32 candidate_count,
...
static void VK_DepthResourcesCreate(VulkanContext *vk_ctx,
  ...
  VkFormat depth_format = VK_SupportedFormat(
      vk_ctx->physical_device, depth_formats, ArrayCount(depth_formats),
  ...
  ImageAllocation image_alloc = ImageAllocationCreate(
      ...
      swapchain_resources->swapchain_extent.height, vk_ctx->msaa_samples,
  ...
  ImageViewResource image_view_resource =
      ImageViewResourceCreate(vk_ctx->device, image_alloc.image, depth_format,
                              VK_IMAGE_ASPECT_DEPTH_BIT, 1);

  swapchain_resources->depth_image_resource = {
      .image_alloc = image_alloc, .image_view_resource = image_view_resource};
}

static void ObjectIdImageResourceCreate(SwapchainResources *swapchain_resources,
                                        U32 image_count) {
  ...
  VkFormat attachment_format =
      VK_SupportedFormat(vk_ctx->physical_device, attachment_formats,
  ...
  for (U32 i = 0; i < image_count; i++) {
    ...
    *image_resolve_alloc = ImageAllocationCreate(
        vk_ctx->allocator, swapchain_resources->swapchain_extent.width,
        swapchain_resources->swapchain_extent.height, VK_SAMPLE_COUNT_1_BIT,
        attachment_format, tiling,
        VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT |
            VK_IMAGE_USAGE_TRANSFER_SRC_BIT,
        1, vma_info);

    *image_resolve_view_resource = ImageViewResourceCreate(
        vk_ctx->device, image_resolve_alloc->image, attachment_format,
        VK_IMAGE_ASPECT_COLOR_BIT, 1);
  ...
  swapchain_resources->object_id_image_resolve_resources =
      object_id_image_resolve_resources;

  BufferReadbackCreate(vk_ctx->allocator, buffer_size,
                       VK_BUFFER_USAGE_TRANSFER_DST_BIT,
                       &swapchain_resources->object_id_buffer_readback);
}

static VkCommandPool VK_CommandPoolCreate(VkDevice device,
...
static void VK_SurfaceCreate(VulkanContext *vk_ctx, IO *io) {
  int supported = glfwVulkanSupported();
  if (supported != GLFW_TRUE) {
    exitWithError("Vulkan loader or ICD loading failed!");
  }

  VkResult result = glfwCreateWindowSurface(vk_ctx->instance, io->window,
                                            nullptr, &vk_ctx->surface);
  if (result != VK_SUCCESS) {
    exitWithError("failed to create window surface!");
  }
}

static void VK_CreateInstance(VulkanContext *vk_ctx) {
  Temp scratch = ScratchBegin(0, 0);
  if (vk_ctx->enable_validation_layers &&
      !VK_CheckValidationLayerSupport(vk_ctx)) {
  ...
  createInfo.ppEnabledExtensionNames =
      CStrArrFromStr8Buffer(scratch.arena, extensions);
...
static void VK_LogicalDeviceCreate(Arena *arena, VulkanContext *vk_ctx) {
  QueueFamilyIndices queueFamilyIndicies = vk_ctx->queue_family_indices;

  U32 uniqueQueueFamiliesCount = 1;
  if (queueFamilyIndicies.graphicsFamilyIndex !=
  ...
  if (vkCreateDevice(vk_ctx->physical_device, &createInfo, nullptr,
                     &vk_ctx->device) != VK_SUCCESS) {
  ...
  vkCmdSetColorWriteEnableEXT =
      (PFN_vkCmdSetColorWriteEnableEXT)vkGetDeviceProcAddr(
...
static void VK_PhysicalDevicePick(VulkanContext *vk_ctx) {
  Temp scratch = ScratchBegin(0, 0);
  ...
  VkPhysicalDevice *devices =
      PushArray(scratch.arena, VkPhysicalDevice, deviceCount);
  vkEnumeratePhysicalDevices(vk_ctx->instance, &deviceCount, devices);

  for (U32 i = 0; i < deviceCount; i++) {
    QueueFamilyIndexBits familyIndexBits =
        VK_QueueFamiliesFind(vk_ctx, devices[i]);
...
static bool VK_CheckValidationLayerSupport(VulkanContext *vk_ctx) {
  Temp scratch = ScratchBegin(0, 0);
  uint32_t layerCount;
  vkEnumerateInstanceLayerProperties(&layerCount, nullptr);

  VkLayerProperties *availableLayers =
      PushArray(scratch.arena, VkLayerProperties, layerCount);
...
static Buffer<String8> VK_RequiredExtensionsGet(VulkanContext *vk_ctx) {
  ...
  Buffer<String8> extensions =
      BufferAlloc<String8>(vk_ctx->arena, extensionCount);
...
static void VK_DebugMessengerSetup(VulkanContext *vk_ctx) {
  if (!vk_ctx->enable_validation_layers)
    return;
...
static void VK_PopulateDebugMessengerCreateInfo(
    VkDebugUtilsMessengerCreateInfoEXT &createInfo) {
  createInfo = {};
...
static bool VK_CheckDeviceExtensionSupport(VulkanContext *vk_ctx,
...
static void VK_ColorResourcesCleanup(VmaAllocator allocator,
                                     ImageResource color_image_resource) {
  ImageResourceDestroy(allocator, color_image_resource);
}

static void VK_DepthResourcesCleanup(VmaAllocator allocator,
                                     ImageResource depth_image_resource) {
  ImageResourceDestroy(allocator, depth_image_resource);
}

static void ObjectIdResourcesCleanup() {
  VulkanContext *vk_ctx = VulkanCtxGet();
  SwapchainResources *swapchain_resources = vk_ctx->swapchain_resources;
  for (U32 i = 0; i < swapchain_resources->object_id_image_resources.size;
       i++) {
    ImageResourceDestroy(
        vk_ctx->allocator,
  ...
  BufferReadbackDestroy(vk_ctx->allocator,
                        &swapchain_resources->object_id_buffer_readback);
}

static void VK_SyncObjectsCreate(VulkanContext *vk_ctx) {
  ...
  vk_ctx->render_finished_semaphores =
      BufferAlloc<VkSemaphore>(vk_ctx->arena, MAX_FRAMES_IN_FLIGHT);
  vk_ctx->in_flight_fences =
      BufferAlloc<VkFence>(vk_ctx->arena, MAX_FRAMES_IN_FLIGHT);
  ...
  for (U32 i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
    if ((vkCreateSemaphore(vk_ctx->device, &semaphoreInfo, nullptr,
                           &vk_ctx->image_available_semaphores.data[i]) !=
         VK_SUCCESS) ||
        (vkCreateSemaphore(vk_ctx->device, &semaphoreInfo, nullptr,
                           &vk_ctx->render_finished_semaphores.data[i]) !=
         VK_SUCCESS) ||
...
static void SyncObjectsDestroy(VulkanContext *vk_ctx) {
  for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
    vkDestroySemaphore(vk_ctx->device,
                       vk_ctx->image_available_semaphores.data[i], nullptr);
    vkDestroySemaphore(vk_ctx->device,
                       vk_ctx->render_finished_semaphores.data[i], nullptr);
    vkDestroyFence(vk_ctx->device, vk_ctx->in_flight_fences.data[i], nullptr);
  }
}

static void VK_CommandBuffersCreate(VulkanContext *vk_ctx) {
  vk_ctx->command_buffers =
      BufferAlloc<VkCommandBuffer>(vk_ctx->arena, MAX_FRAMES_IN_FLIGHT);
  ...
  if (vkAllocateCommandBuffers(vk_ctx->device, &allocInfo,
                               vk_ctx->command_buffers.data) != VK_SUCCESS) {
    exitWithError("failed to allocate command buffers!");
  }
}

static ShaderModuleInfo ShaderStageFromSpirv(Arena *arena, VkDevice device,
...
static VkShaderModule ShaderModuleCreate(VkDevice device, Buffer<U8> buffer) {
...
static void BufferDestroy(VmaAllocator allocator,
...
static void BufferMappedDestroy(VmaAllocator allocator,
...
BufferUploadDevice(VkCommandBuffer cmd_buffer, BufferAllocation staging_buffer,
                   VulkanContext *vk_ctx, Buffer<T> buffer_host,
  ...
  wrapper::BufferAllocation buffer = wrapper::BufferAllocationCreate(
      vk_ctx->allocator, staging_buffer.size,
      usage | VK_BUFFER_USAGE_TRANSFER_DST_BIT, vma_info);

  vmaCopyMemoryToAllocation(vk_ctx->allocator, buffer_host.data,
                            staging_buffer.allocation, 0, staging_buffer.size);

  VkBufferCopy copy_region = {0};
  copy_region.size = staging_buffer.size;
  vkCmdCopyBuffer(cmd_buffer, staging_buffer.buffer, buffer.buffer, 1,
                  &copy_region);

  return buffer;
}

static BufferAllocation StagingBufferCreate(VmaAllocator allocator,
                                            VkDeviceSize size) {
...
static void BufferAllocCreateOrResize(VmaAllocator allocator,
                                      U32 total_buffer_byte_count,
  ...
  if (total_buffer_byte_count > buffer_alloc->size) {
    ...
    *buffer_alloc = BufferAllocationCreate(allocator, total_buffer_byte_count,
                                           usage, vma_info);
  }
}
...
VK_QueueFamilyIndicesFromBitFields(QueueFamilyIndexBits queueFamilyBits) {
  ...
  indices.graphicsFamilyIndex =
      (U32)LSBIndex((S32)queueFamilyBits.graphicsFamilyIndexBits);
  indices.presentFamilyIndex =
      (U32)LSBIndex((S32)queueFamilyBits.presentFamilyIndexBits);

  return indices;
}

static QueueFamilyIndexBits VK_QueueFamiliesFind(VulkanContext *vk_ctx,
                                                 VkPhysicalDevice device) {
  Temp scratch = ScratchBegin(0, 0);
  ...
  for (U32 i = 0; i < queueFamilyCount; i++) {
    ...
    vkGetPhysicalDeviceSurfaceSupportKHR(device, i, vk_ctx->surface,
                                         &presentSupport);
    if (presentSupport) {
  ...
  U32 sameFamily =
      indices.graphicsFamilyIndexBits & indices.presentFamilyIndexBits;
  if (sameFamily) {
...
static bool VK_IsDeviceSuitable(VulkanContext *vk_ctx, VkPhysicalDevice device,
  ...
  return VK_QueueFamilyIsComplete(indexBits) && extensionsSupported &&
         swapChainAdequate && supportedFeatures.samplerAnisotropy;
}

static bool VK_QueueFamilyIsComplete(QueueFamilyIndexBits queueFamily) {
  if ((!queueFamily.graphicsFamilyIndexBits) ||
      (!queueFamily.presentFamilyIndexBits)) {
    return false;
  }
  return true;
}

static SwapChainSupportDetails VK_QuerySwapChainSupport(Arena *arena,
                                                        VkPhysicalDevice device,
  ...
  U32 formatCount;
  VK_CHECK_RESULT(vkGetPhysicalDeviceSurfaceFormatsKHR(device, surface,
                                                       &formatCount, nullptr));
  ...
  if (presentModeCount != 0) {
    details.presentModes =
        BufferAlloc<VkPresentModeKHR>(arena, presentModeCount);
    VK_CHECK_RESULT(vkGetPhysicalDeviceSurfacePresentModesKHR(
...
static void FrustumPlanesCalculate(Frustum *out_frustum,
...
static void ImageViewResourceDestroy(ImageViewResource image_view_resource) {
  vkDestroyImageView(image_view_resource.device, image_view_resource.image_view,
                     0);
}

static void ImageAllocationDestroy(VmaAllocator allocator,
                                   ImageAllocation image_alloc) {
  vmaDestroyImage(allocator, image_alloc.image, image_alloc.allocation);
}

static void ImageResourceDestroy(VmaAllocator allocator, ImageResource image) {
  ImageViewResourceDestroy(image.image_view_resource);
  ImageAllocationDestroy(allocator, image.image_alloc);
}

static BufferAllocation
BufferAllocationCreate(VmaAllocator allocator, VkDeviceSize size,
  ...
  VK_CHECK_RESULT(vmaCreateBuffer(allocator, &bufferInfo, &vma_info,
                                  &buffer.buffer, &buffer.allocation, nullptr));

  return buffer;
}

static void BufferReadbackCreate(VmaAllocator allocator, VkDeviceSize size,
                                 VkBufferUsageFlags buffer_usage,
  ...
  VK_CHECK_RESULT(vmaCreateBuffer(allocator, &bufCreateInfo, &allocCreateInfo,
                                  &buf, &alloc, &allocInfo));

  out_buffer_readback->mapped_ptr = allocInfo.pMappedData;
  out_buffer_readback->buffer_alloc = {
      .buffer = buf, .allocation = alloc, .size = size};
}

static void BufferReadbackDestroy(VmaAllocator allocator,
                                  BufferReadback *out_buffer_readback) {
...
BufferMappedCreate(VmaAllocator allocator, VkDeviceSize size,
                   VkBufferUsageFlags buffer_usage) {
  ...
  BufferAllocation buffer = BufferAllocationCreate(
      allocator, size, buffer_usage | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
      vma_info);

  VkMemoryPropertyFlags mem_prop_flags;
  vmaGetAllocationMemoryProperties(allocator, buffer.allocation,
                                   &mem_prop_flags);
  ...
  if (!mapped_buffer.mapped_ptr) {
    ...
    mapped_buffer.staging_buffer_alloc = BufferAllocationCreate(
        allocator, size, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, vma_info);
  }

  return mapped_buffer;
}

static void BufferMappedUpdate(VkCommandBuffer cmd_buffer,
                               VmaAllocator allocator,
                               BufferAllocationMapped mapped_buffer) {
  if (mapped_buffer.mem_prop_flags & VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT) {
    if ((mapped_buffer.mem_prop_flags & VK_MEMORY_PROPERTY_HOST_COHERENT_BIT) ==
        0) {
    ...
    if (vmaCopyMemoryToAllocation(allocator, mapped_buffer.mapped_ptr,
                                  mapped_buffer.staging_buffer_alloc.allocation,
                                  0, mapped_buffer.buffer_alloc.size)) {
      exitWithError(
          "BufferMappedUpdate: Could not copy data to staging buffer");
    }
...
static ImageViewResource ImageViewResourceCreate(VkDevice device, VkImage image,
                                                 VkFormat format,
...
ImageAllocationCreate(VmaAllocator allocator, U32 width, U32 height,
                      ...
                      U32 mipmap_level, VmaAllocationCreateInfo vma_info) {
  ...
  VmaAllocationInfo alloc_info;
  VK_CHECK_RESULT(vmaCreateImage(allocator, &image_create_info, &vma_info,
                                 &image_alloc.image, &image_alloc.allocation,
                                 &alloc_info))
...
static void VK_ColorResourcesCreate(VulkanContext *vk_ctx,
                                    SwapchainResources *swapchain_resources) {
  ...
  ImageAllocation image_alloc = ImageAllocationCreate(
      ...
      VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT, 1,
      vma_info);

  ImageViewResource color_image_view =
      ImageViewResourceCreate(vk_ctx->device, image_alloc.image, colorFormat,
                              VK_IMAGE_ASPECT_COLOR_BIT, 1);

  swapchain_resources->color_image_resource = {
      .image_alloc = image_alloc, .image_view_resource = color_image_view};
}
static void SwapChainImageResourceCreate(
    VkDevice device, SwapchainResources *swapchain_resources, U32 image_count) {
  ScratchScope scratch = ScratchScope(0, 0);

  swapchain_resources->image_resources = BufferAlloc<ImageSwapchainResource>(
      swapchain_resources->arena, image_count);

  VkImage *images = PushArray(scratch.arena, VkImage, image_count);
  if (vkGetSwapchainImagesKHR(device, swapchain_resources->swapchain,
                              &image_count, images) != VK_SUCCESS) {
  ...
  for (uint32_t i = 0; i < image_count; i++) {
    ...
    swapchain_resources->image_resources.data[i] = {
        .image = images[i], .image_view_resource = image_view_resource};
  }
}
...
VK_ChooseSwapSurfaceFormat(Buffer<VkSurfaceFormatKHR> availableFormats) {
  for (U32 i = 0; i < availableFormats.size; i++) {
    if (availableFormats.data[i].format == VK_FORMAT_R8G8B8A8_SRGB &&
        availableFormats.data[i].colorSpace ==
...
VK_ChooseSwapExtent(IO *io_ctx, const VkSurfaceCapabilitiesKHR &capabilities) {
  if (capabilities.currentExtent.width != UINT32_MAX) {
    ...
    VkExtent2D actualExtent = {(U32)io_ctx->framebuffer_width,
                               (U32)io_ctx->framebuffer_height};

    actualExtent.width =
        Clamp(capabilities.minImageExtent.width, actualExtent.width,
              capabilities.maxImageExtent.width);
    actualExtent.height =
        Clamp(capabilities.minImageExtent.height, actualExtent.height,
...
static U32 VK_SwapChainImageCountGet(VkDevice device,
                                     SwapchainResources *swapchain_resources) {
  U32 imageCount = {0};
  if (vkGetSwapchainImagesKHR(device, swapchain_resources->swapchain,
...
static SwapchainResources *VK_SwapChainCreate(VulkanContext *vk_ctx,
                                              IO *io_ctx) {
  Arena *arena = ArenaAlloc();
  SwapchainResources *swapchain_resources =
      PushStruct(arena, SwapchainResources);
  ...
  VkSurfaceFormatKHR surface_format =
      VK_ChooseSwapSurfaceFormat(swapchain_details.formats);
  VkPresentModeKHR present_mode =
      VK_ChooseSwapPresentMode(swapchain_details.presentModes);
  VkExtent2D swapchain_extent =
      VK_ChooseSwapExtent(io_ctx, swapchain_details.capabilities);
  ...
  if (queueFamilyIndices.graphicsFamilyIndex !=
    ...
    U32 queueFamilyIndicesSame[] = {
        vk_ctx->queue_family_indices.graphicsFamilyIndex,
  ...
  VK_CHECK_RESULT(
      vkCreateSwapchainKHR(vk_ctx->device, &createInfo, nullptr, &swapchain));
  swapchain_resources->swapchain = swapchain;
  swapchain_resources->swapchain_extent =
      swapchain_details.capabilities.currentExtent;
  ...
  U32 swapchain_image_count =
      VK_SwapChainImageCountGet(vk_ctx->device, swapchain_resources);
  SwapChainImageResourceCreate(vk_ctx->device, swapchain_resources,
...
static void VK_SwapChainCleanup(VkDevice device, VmaAllocator allocator,
                                SwapchainResources *swapchain_resources) {
  VK_ColorResourcesCleanup(allocator,
                           swapchain_resources->color_image_resource);
  VK_DepthResourcesCleanup(allocator,
                           swapchain_resources->depth_image_resource);
  ObjectIdResourcesCleanup();

  for (size_t i = 0; i < swapchain_resources->image_resources.size; i++) {
    ImageViewResourceDestroy(
...
static void VK_RecreateSwapChain(IO *io_ctx, VulkanContext *vk_ctx) {
  VK_CHECK_RESULT(vkDeviceWaitIdle(vk_ctx->device));

  SyncObjectsDestroy(vk_ctx);
  VK_SwapChainCleanup(vk_ctx->device, vk_ctx->allocator,
...
// Samplers helpers
//
static VkSampler SamplerCreate(VkDevice device,
...
static void DescriptorPoolCreate(VulkanContext *vk_ctx) {
  ...
  if (vkCreateDescriptorPool(vk_ctx->device, &poolInfo, nullptr,
                             &vk_ctx->descriptor_pool) != VK_SUCCESS) {
    exitWithError("failed to create descriptor pool!");
  }
}
static VkDescriptorSetLayout
DescriptorSetLayoutCreate(VkDevice device,
...
static R_Handle VK_DescriptorSetCreate(Arena *arena, VkDevice device,
  ...
  texture_sampler_desc.descriptorType =
  ...
  vkUpdateDescriptorSets(device, ArrayCount(descriptors), descriptors, 0,
                         nullptr);

  return R_HandleFromPtr(desc_set);
}

static R_Handle DescriptorSetCreate(Arena *arena, VkDevice device,
                                    VkDescriptorPool desc_pool,
                                    VkDescriptorSetLayout desc_set_layout,
                                    Texture *texture) {
  return VK_DescriptorSetCreate(
      arena, device, desc_pool, desc_set_layout,
      texture->image_resource.image_view_resource.image_view, texture->sampler);
}

static VkFilter VkFilterFromFilter(R_Filter filter) {
  switch (filter) {
...
// ~mgj: Sampler
static void
VkSamplerCreateInfoFromSamplerInfo(R_SamplerInfo *sampler,
  ...
  out_sampler_info->mipmapMode =
      VkSamplerMipmapModeFromMipMapMode(sampler->mip_map_mode);
...
static VkFormat R_VkFormatFromTex2DFormat(R_Tex2DFormat format) {
